...
Function /DF init_fit_panel()
	...
	Color_New10_curve[][9,16]=Color_New10_curve[p][q-8]
    	...
    	// This is where the number of the Peak, Bg, and Parameter function is set
...
Function open_fit_panel(ctrlName)
   ...
   Variable Width = 650 *panelscale*MacScale//* SC		// panel size  
	Variable height = 450 *panelscale*MacScale//* SC
	...
	Button global_close_panel, pos={500*SC,4*SC},size={60*SC,18*SC},title="close", proc=close_fit_panel
	Button global_duplicate_panel, size={80*SC,18*SC}, pos={560*SC,4*SC},title="New Fit", proc=open_fit_panel
	
	//Comment out by W. S. Lee 01/26/2023
...
//	Button Fit_bt8,size={60*SC,20*SC},pos={340*SC,170*SC},title="SavePar",disable=1,proc=fitpanel#save_curvefit_coef
	...
	//Commented by W. S. Lee
	PopupMenu output_pp0,size={100*SC,20*SC},pos={395*SC,112*SC},title="xwave:",disable=0,mode=1,value="No;Yes", noproc, help={"When selecting YES, please make sure xwave is created in the assocaited fit_panel folder."}
	...
	button output_b9,size={55*SC,18*SC},pos={570*SC,115*SC},title="SaveDisp",disable=1,proc=fitpanel#FitOutput_SaveDisp
	
	
	groupbox Output_gb1,frame=0,size={300*SC,55*SC},pos={330*SC,135*SC},title="display result Image",disable=1
	
	Setvariable Output_sv1,size={90*SC,20*SC},pos={340*SC,152*SC},limits={-inf,inf,0},title="pnts:",value=DFR_panel:gv_imageresultpnts,disable=1
	
	button output_b11,size={65*SC,20*SC},pos={440*SC,152*SC},title="Image disp",disable=1,proc=fitpanel#FitOutput_DisplayImage
	button output_b12,size={55*SC,20*SC},pos={510*SC,152*SC},title="Raw disp",disable=1,proc=fitpanel#FitOutput_DisplayImage
	button output_b13,size={55*SC,20*SC},pos={570*SC,152*SC},title="Fit disp",disable=1,proc=fitpanel#FitOutput_DisplayImage
	//Added by W. S. Lee for option to plotted peak with resolution convolution
	checkbox Output_ck4, size={55*SC,18*SC},pos={440*SC,170*SC},title="Conv?",disable=0,value=1 
...
static Function fit_AutoTab( name, tab )
    ...
    if (stringmatch(tabstr,"Output")==1)
   		...
   		if (dimflag==2)
   			PopupMenu output_pp0,disable=0
   		endif
...
Function update_fit_wave(flag)
	...
	if (flag==0)		
		...
		//Commented by W.S. Lee 01/23/2023
...
// The following switch is simplfied by W. S. Lee 01/26/2023
	Strswitch(ctrlname)
	case "Function_ck1":
		savestr="Convolve=Convolve"
	break
...
static function FitOutput_DisplayImage(ctrlname)
	...
	if (numtype(gv_imageresultpnts)==2)
		...
		Wave w_result_temp=$ImagePlotname
		Setscale /P x, x0,dx,w_result_temp
		Setscale /I y, y0,y0+((Yn-1)*dy),w_result_temp
	endif
...
//			added by W.S. Lee 02/20/2023 for plotting the peak convoluted with resolution
			
			WAVE /Z SavePar_Convolve = DFR_fit:SavePar_Convolve
			controlinfo Output_ck4
			if (V_value ==1 && Waveexists(SavePar_Convolve)==1)
	
				variable fwhm = SavePar_Convolve[index]
				variable gauss_from = 10*fwhm
				variable scalex = abs(deltax(w_trace_temp))
				Variable gauss_pnts = round(gauss_from/scalex) * 2 + 1
				Make/o/d/n=(gauss_pnts) w_conv_gauss_plot = 0
				setscale /P x -gauss_from, scalex, w_conv_gauss_plot
				w_conv_gauss_plot= exp(-x^2*4*ln(2)/fwhm^2)
   			Variable sumGauss = sum(w_conv_gauss_plot, -inf,inf)
    			w_conv_gauss_plot /= sumGauss
			
				convolve /A w_conv_gauss_plot w_trace_temp  
			endif
	...
	Killwaves /Z w_y0,w_dy,w_yn,w_result_Temp,w_trace_Temp,w_conv_gauss_plot
	
	SetDatafolder DF
	
End

static function FitOutput_SaveDisp(ctrlname)  //Added by W. S. Lee 02/20/2023 to save Dispersion correction curve
string ctrlname
	...
	Variable vertflag=v_value
		...
		String w_disp="root:RIXS_NormWaves:"
		SetDatafolder $w_disp
		Savewavename += "_disp"
...
//				break
//			case 2:
//				xwaveN=Savewavename+"_xwave"
//				wave xwave=DFR_panel:xwave  //put your xwave name in the corresponding fit_panel folder
...
//			endswitch
			...
			if (V_value==1)
				...
				string xwaveN=Savewavename+"_xwave"
				wave /Z xwave=DFR_panel:xwave  //put your xwave name in the corresponding fit_panel folder
				if (waveExists(xwave))
					duplicate /o xwave, $xwaveN
					wave xwave=$xwaveN
				endif
		...
		ModifyGraph marker=19,msize=4,mrkThick=0.25,useMrkStrokeRGB=1
		SetAxis bottom *,2

...
//	SetVariable source_file_name,pos={20,70},size={240,20},fsize=14,title="Source file name"
	...
	CheckBox setscale_cb, pos={400, 400}, fsize=14, title="set scale"
...
static Function LoadFileProc(B_Struct) : ButtonControl
   ...
   EndIf
	...
	controlinfo setscale_cb
	
	//The following action is added by W. S. Lee 05/21/2023, it will set scale automatically with the wave selected in the list box, only works for data with even spacing
	if (V_Value == 1)
		String path_listWave = "root:RIXS_Analysis_Macro_System_File:listbox_listWave"
		String path_selWave = "root:RIXS_Analysis_Macro_System_File:listbox_selWave"
		wave/T listbox_listWave = $path_listWave
		...
		do //this loop determine which wave is selected as the xwave for scaling
			if (listbox_selWave[index][1] != 2)
			sel = index 
			endif
			index= index + 1
		while(index < dimsize(listbox_selWave, 0))
		string xwave_nm = wname_prefix+"_"+listbox_listWave[sel][1]
		wave xwave=$xwave_nm
		
		//setscale all the waves
		index = 0
		do 
			string ywave_nm = wname_prefix+"_"+listbox_listWave[index][1]
			wave ywave=$ywave_nm
			setscale /I x, xwave[0], xwave[numpnts(xwave)-1], ywave
			index = index +1
		while (index < dimsize(listbox_listWave, 0))	
	endif
